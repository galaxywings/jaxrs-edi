  ...
  export default {
    ...
    computed: {
      oneFileSelected: function () {
        return this.selectedItems.length === 1 && this.selectedItems[0].filename !== this.files[0].filename
      },
      ...
      restoreFile: _.debounce(function () {
        ...
        this.$http[method](url, params)
          .then(({data}) => {
            this.$notify.success({
              title: '成功',
              message: `模板文件${action}成功!`
            })
            this.$emit('updateFilename', params.filename)
            this.loadFiles()
          },
          (response) => {
